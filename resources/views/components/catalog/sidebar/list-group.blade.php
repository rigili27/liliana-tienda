<div>
    ...
    <div class="flex max-w-xs">
        <p class="flex-1 text-lg font-bold pb-2 text-gray-800 dark:text-white">Categories</p>
        @if (Cache::get('category_filter') != '')
            <span class="text-lg text-red-600 cursor-pointer" wire:click="changeFilterCategory('')">Clear filter</span>
        @endif
    </div>
    <ul class="max-w-xs flex flex-col max-h-96 overflow-y-auto">
        @foreach ($categories as $item)
            <x-catalog.sidebar.list :name="$item->name" wire:click="changeFilterCategory({{ $item->id }})" class="{{ Cache::get('category_filter') === $item->id ? 'text-blue-800' : 'text-gray-800' }} cursor-pointer" />
        @endforeach
    </ul>
</div>
