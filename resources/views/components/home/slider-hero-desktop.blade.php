        ...
        <div class="grid grid-cols-5 .mx-auto .w-[70vw] h-[60vh] mt-7 gap-4 .bg-red-200">
            ...
            <div class="h-[60vh]">
                <ul class="max-w-xs flex flex-col overflow-y-auto h-full scrollbar-hide shadow-md">
                    ...
                    <li wire:click="goToCatalog()" class="hover:bg-gray-100 inline-flex cursor-pointer items-center gap-x-3.5 py-3 px-4 text-sm font-medium bg-white border border-gray-200 -mt-px first:mt-0 first:rounded-t-md uppercase">Catalogo completo</li>

                    @foreach ($listattributes as $a)
                        <li wire:click="goToCatalog({{ $a->id }})" class="hover:bg-gray-100 inline-flex cursor-pointer items-center gap-x-3.5 py-3 px-4 text-sm font-medium bg-white  border border-gray-200 -mt-px first:mt-0  first:rounded-t-md uppercase">{{ $a->show_name }}</li>
                    @endforeach

                    @foreach ($families as $item)
                        <li wire:click="goToCatalog('', {{ $item->id }})" class="hover:bg-gray-100 cursor-pointer inline-flex items-center gap-x-3.5 py-3 px-4 text-sm font-medium bg-white border border-gray-200 -mt-px first:mt-0 last:rounded-b-md uppercase">{{ $item->name }}</li>
                    @endforeach
                </ul>
